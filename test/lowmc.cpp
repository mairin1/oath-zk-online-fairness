...
void test_lowmc(BoolIO<NetIO> *ios[threads], int party) {
  ...
  uint64_t comm_start = ios[0]->io->counter;
  auto start = clock_start();
  lowmc->encrypt(ctx, ptx, nblocks);
  double tt = time_from(start);
  uint64_t comm_end = ios[0]->io->counter;
  uint64_t tot_comm = comm_end - comm_start;
  ...
  std::cout << "party: " << party << "\t comm: " << tot_comm << "\n";
